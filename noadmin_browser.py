...
class Window(QMainWindow):
	...
	def init_ui(self):
		...
		self.profile_pic.setPixmap(student_pic)
	...
	def dlt(self):
		...
		if form.exec_() == QDialog.Accepted:
			print("Это секретное сообщение, оно видно только в консоли!")
		self.refresh_data()
	
	def docs_view(self):
		path_to_current_doc = Window.DATABASE.query(f"select path from Docs where storage_id = "
		                                            f"(select id from Storage where name = "
		                                            f"'{self.profiles.selectedItems()[0].text()}')")
